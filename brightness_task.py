...
class ContrastExperiment:
    def __init__(self):
        self.root = tk.Tk()
        self.root.title("brightness-task")
    ...
    def show_instructions(self):
        ...
        Verwenden Sie die PFEILTASTEN um die Helligkeit des Bildschirmes anzupassen:
...
Bitte stellen Sie die HELLIGKEIT DES BILDSCHIRMS so ein,
...
mit der Enter- oder der Leertaste."""
    ...
    def save_results(self):
        ...
        #filename = f"data\\contrast_task_brightness_{datetime.now().strftime('%Y%m%d_%H%M%S')}.txt"
        filename = os.path.join(data_dir, f"{SessionData[1]}_brightness_{SessionData[0]}_{SessionData[2]}_{datetime.now().strftime('%Y%m%d_%H%M%S')}.txt")
        try:
