...
class ContrastExperiment:
    def __init__(self):
        self.root = tk.Tk()
        self.root.title("contrast-task - blackbar")
...
mit der Enter- oder der Leertaste."""
    ...
    def save_results(self):
        ...
        #filename = f"data\\contrast_task_blackbar_{datetime.now().strftime('%Y%m%d_%H%M%S')}.txt"
        filename = os.path.join(data_dir, f"{SessionData[1]}_blackbar_{SessionData[0]}_{SessionData[2]}_{datetime.now().strftime('%Y%m%d_%H%M%S')}.txt")
        try:
